...
//fetching current precedent tasks
$precedentsforminputs = '';
$currprecedentsdivs = '';
$q = db_query('SELECT precedents FROM ' . PRE . 'tasks WHERE id =' . $TASKID_ROW['id']);
//$currprecedentsdivs = 'SELECT precedents FROM ' . PRE . 'tasks WHERE id =' . $TASKID_ROW['id'];
for ($i = 0; $task_prec = @db_fetch_array($q, $i); ++$i) {
    $arrcurrprec = ($task_prec['precedents'] != "") ? explode(",", $task_prec['precedents']) : [];
    if (count($arrcurrprec) > 0) {
        $precedentsq = 'SELECT t.id, CONCAT(parentnames.task_name,">",t.task_name, " ",DATE_FORMAT(deadline,"%d-%m-%Y")) compeltename, parent, projectid FROM ' . PRE . 'tasks t LEFT JOIN (SELECT id,task_name FROM ' . PRE . 'tasks) parentnames ON t.parent=parentnames.id WHERE t.id IN (' . $task_prec['precedents'] . ') ORDER BY deadline DESC';
        $q = db_query($precedentsq);
        for ($i = 0; $task_row_prec = @db_fetch_array($q, $i); ++$i) {
            $precedentsforminputs .= '<input id="currprecid' . $task_row_prec['id'] . '" type="hidden" name="precedentids[]" value="' . $task_row_prec['id'] . '"/>';
            $currprecedentsdivs .= "<div style='padding:3px;' precid='" . $task_row_prec['id'] . "'>" . $task_row_prec["compeltename"] . " <a style='cursor:pointer;color:orangered;font-size:70%;' onclick='removeprecedent($(this).parent());'>Eliminar</a></div>";
        }
    }
}
//end of current precedents fetching
//start showing task info
$content .= "<form method=\"post\" action=\"tasks.php\" onsubmit= \"return fieldCheck('name') && dateCheck() && precedentsCheck(this);\">\n" .
...
//precendents or constraints functionality
$content .= "<tr><td>Precedencias</td><td><select id='selprecedents' onchange='addPrecedent();'><option value='0'></option>";

$q = db_query('SELECT t.id, CONCAT(IF(parentnames.parent=0,"",CONCAT(LEFT(parentnames.task_name,5),">")),t.task_name, " ",DATE_FORMAT(deadline,"%d-%m-%Y")) compeltename, t.parent, projectid FROM ' . PRE . 'tasks t LEFT JOIN (SELECT id,task_name,parent FROM ' . PRE . 'tasks) parentnames ON t.parent=parentnames.id WHERE t.parent NOT IN (0) AND projectid=' . $TASKID_ROW['projectid'] . ' ORDER BY parentnames.task_name,t.task_name');
for ($i = 0; $task_row = @db_fetch_array($q, $i); ++$i) {
    $content .= "<option value=\"" . $task_row['id'] . "\"";

    if ($TASKID_ROW['taskgroupid'] == $task_row['id']) {
        $content .= " selected=\"selected\"";
    }
    $content .= ">" . $task_row['compeltename'] . "</option>\n";
}

$content .= "</select></td></tr>" .
        "<tr><td></td><td><div id='precedentstoadd' name='precedentstoadd' style='padding:5px;border:solid 1px rgb(195, 195, 195);'>$currprecedentsdivs</div></td></tr>";
//End of precedents functionality
//
//deadline
