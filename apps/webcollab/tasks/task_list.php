...
function listTasks($parentid) {
    ...
    if ($task_array[0]['group_id']) {
        ...
        $content = "<ol class=\"ul-" . $level_count . "\">\n";
        $taskgroup_flag = 0;
    }
...
function task_state($key) {
    ...
    //add precedents/constraints if there are some
    $precedentsarr = [];
    $arrcurrprec = ($task_array[$key]['precedents'] != "") ? explode(",", $task_array[$key]['precedents']) : [];
    if (count($arrcurrprec) > 0) {
        $precedentsq = 'SELECT t.id, CONCAT(IF(parentnames.parent=0,"",CONCAT(LEFT(parentnames.task_name,5),">")),LEFT(t.task_name,20)) compeltename,UNIX_TIMESTAMP(t.deadline) precdue, t.parent, projectid FROM ' . PRE . 'tasks t LEFT JOIN (SELECT id,task_name,parent FROM ' . PRE . 'tasks) parentnames ON t.parent=parentnames.id WHERE t.id IN (' . implode(",", $arrcurrprec) . ') ORDER BY deadline DESC';
        ...
        for ($i = 0; $task_row_prec = @db_fetch_array($q, $i); ++$i) {
            ...
            $precedentsarr[] = $task_row_prec['compeltename'] . " " . $precdue;
        }
    }
    $content .= (count($precedentsarr) > 0) ? " <span style='color:#ce494a;'>" . implode(" ,", $precedentsarr) . "</span>" : "";
...
if ($i > 0) {
    ...
    $content .= '
<style>
ol li {display:block;padding-top:5px;} /* hide original list counter */
ol > li:first-child {counter-reset: item;} /* reset counter */
ol > li {counter-increment: item; position: relative;} /* increment counter */
ol > li:before {content:counters(item, ".") ". "; position: absolute; margin-right: 100%; right: 5px;color:#003399;} /* print counter */
            </style>
