...
$precedentids = array_unique($_POST["precedentids"], SORT_NUMERIC);
$strprecedentids = "";
if (count($precedentids) > 0) {
    $strprecedentids = implode(",", $precedentids);
}
...
$q = db_prepare('INSERT INTO ' . PRE . 'tasks(task_name,
                                          ...
                                          task_status,
                                          sequence,
                                          completion_time,precedents )
              values(?, ?, now(), now(), now(), now(), ?, ?, ?, now(), ?, ?, ?, ?, ?, ?, ?, ?, 0, now(),? )');

db_execute($q, array($name, $text, $owner, UID, $deadline, $priority, $parentid, $projectid, $taskgroupid, $usergroupid, $globalaccess, $groupaccess, $status, $strprecedentids));
