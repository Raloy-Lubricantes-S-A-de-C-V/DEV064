...
//        ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ...
    function handleMove(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            ...
            var pi = neighbor.pointIndex -1;
            var insa = [si, pi, neighbor.data, plot.series[si].gridData[pi][2]];
//            $.each(insa, function(index, val) {
//                alert(index+"@"+val);
//            });
            var indexes = [];
            //leyenda de la serie
            var insLabel = insa[2][3]["label"];
            $.each(plot["data"][0], function(index, val) {
                var label = val[3]["label"];
                var pos = plot.series[0].gridData[index][2];
                if (label == insLabel) {
                    indexes.push([0, index, val, pos]);
                }
            });
            var canvas = plot.plugins.bubbleRenderer.highlightCanvas;
            var ctx = canvas._ctx;
...
//****
            ...
            evt1.pageX = ev.pageX;
            evt1.pageY = ev.pageY;
            plot.target.trigger(evt1, insa);
            if (plot.series[insa[0]].highlightMouseOver && !(insa[0] == plot.plugins.bubbleRenderer.highlightedSeriesIndex && insa[1] == plot.series[insa[0]]._highlightedPoint)) {
                var evt = jQuery.Event('jqplotDataHighlight');
                evt.which = ev.which;
                evt.pageX = ev.pageX;
                evt.pageY = ev.pageY;
                plot.target.trigger(evt, insa);
            }
//****
